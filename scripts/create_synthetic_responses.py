...
for rater_index in range(1, 3):
    for index, trial in enumerate(trials):
        ...
        append_response(directory, RaterResponse(response_id=response_id, trial_id=trial.trial_id, rater_id=f"synthetic_rater_{rater_index}", selected_character_id=selected, confidence=3, timestamp=datetime.now(timezone.utc), synthetic_data=True))
