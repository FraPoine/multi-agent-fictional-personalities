...
`outputs/poirot/runs/<run_id>/` containing:

- `persona.json`: the mock provider output validated with the `Persona` schema;
- `system_prompt.txt`: the Jinja-rendered Poirot system prompt;
- `response.txt`: the configured mock agent response;
- `metadata.json`: run paths and provenance, including
  `"is_synthetic": true`.

**Warning:** both model outputs are deterministic synthetic development
fixtures. They were not generated by OpenAI and are not evidence that the
persona or response reflects real model behavior. OpenAI execution is still
not available or verified in this pipeline.
